...
export function agentBuilderDeploymentCommands(input: {
  ...
  return [
    ...
    `pnpm sentinel:smoke-test --write-config=${configPath}`,
    `gcloud alpha discovery-engine agents create --project=${input.projectId} --location=${input.region} --display-name=Sentinel --config=${configPath}`,
